(() => {
  ...
  class GraphStore {
    ...
    async scan() {
      ...
      const walk = async (directory, folder = '') => {
        for await (const [name, handle] of directory.entries()) {
          ...
          const journalTitle = folder === 'journals' && /^\d{4}_\d{2}_\d{2}\.(md|markdown)$/i.test(name)
            ? name.replace(/\.(md|markdown)$/i, '').replace(/_/g, '-') : null;
          pages.push({
    ...
    async createPage(title, options = {}) {
      ...
      const name = `${options.filename || safeFilename(title)}.md`;
      const handle = await directory.getFileHandle(name, { create: true });
      const page = { title, name, path: `${folder}/${name}`, folder, handle, content: '', lastModified: 0, journal: folder === 'journals' };
  ...
  function journalInfo(date = new Date()) {
    const year = date.getFullYear();
    const month = String(date.getMonth() + 1).padStart(2, '0');
    const day = String(date.getDate()).padStart(2, '0');
    return { title: `${year}-${month}-${day}`, filename: `${year}_${month}_${day}` };
  }
  ...
  window.MarkdGraph = {
    ...
    saveDraft, getDraft, removeDraft, journalInfo, newId
  };
})();
